...
export default function ProductForm(){
  ...
  useEffect(()=>{
    if(!id)return
    let active=true
    apiRequest(`/admin/products/${id}`).then(payload=>{if(active)setSavedImages(payload.data.images||[])}).catch(requestError=>{if(active)setError(requestError.message)})
    return()=>{active=false}
  },[id])
  ...
  const removeSavedImage=async image=>{
    if(!confirm('Hapus gambar ini dari produk dan Supabase?'))return
    setRemovingImage(image.id);setError('')
    try{
      await apiRequest(`/admin/products/${id}/images/${image.id}`,{method:'DELETE'})
      setSavedImages(current=>current.filter(item=>item.id!==image.id))
      await refresh()
    }catch(requestError){setError(requestError.message||'Gambar belum dapat dihapus.')}finally{setRemovingImage(null)}
  }

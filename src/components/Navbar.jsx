...
export default function Navbar() {
  const { count } = useCart(); const { user } = useAuth(); const [query,setQuery]=useState(''); const [open,setOpen]=useState(false); const navigate=useNavigate()
  ...
  return <header className="sticky top-0 z-50 border-b border-blush/70 bg-warm/95 backdrop-blur"><div className="container-page flex h-18 items-center gap-4">
    <Link to="/" className="focus-ring flex shrink-0 items-center gap-2 rounded-xl" aria-label="Arajut - Beranda"><img src={arajutLogo} alt="" className="h-12 w-12 rounded-full object-cover shadow-sm"/><span><span className="block font-serif text-2xl font-bold leading-none text-primary">Arajut</span><span className="mt-1 hidden text-[9px] tracking-[.16em] text-ink sm:block">HANDMADE WITH LOVE</span></span></Link>
